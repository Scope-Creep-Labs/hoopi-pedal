...
void AmpModule::ProcessMono(float in) {
    ...
    float ampOut;
    float drySignal = m_audioLeft;  // Keep original for dry mix
    float input_arr[1] = {0.0}; // Neural Net Input
    // Gain range: 0.5x to 2.0x (prevents silence at minimum)
    input_arr[0] = m_audioLeft * (0.5f + (m_gainMax - 0.5f) * GetParameterAsFloat(0));
    ...
    // MIX //
    float mix_out = filter_out * wetMix + drySignal * dryMix; // Use original signal for dry mix
