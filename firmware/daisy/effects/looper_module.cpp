...
void LooperModule::AlternateFootswitchPressed() {
    ...
    isRecording = m_looper.Recording();
}
...
void LooperModule::ProcessStereo(float inL, float inR) {
    ...
    const float inputLevel = 0.5f; //m_inputLevelMin + (0.5f * (m_inputLevelMax - m_inputLevelMin));

    const float loopLevel = 0.5f; //m_loopLevelMin + (0.4f * (m_loopLevelMax - m_loopLevelMin));

    float input = m_audioLeft * inputLevel;
    // float inputR = m_audioRight * inputLevel;
